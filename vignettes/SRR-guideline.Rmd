...
<!-- ## 0. 使用データ・解析コード等 -->
  ...
  Hockey-Stick型/Beverton-Holt型/Ricker型の再生産関係を比較します。まずはレジームを考慮しない場合です。
...
## 8. パラメータ間相関
パラメータa, bの値がどれだけ相関を持つか、相関係数を算出します。

```{r,warning=FALSE}
res_corSR <- corSR(resSR=resSR)
res_corSR$cor[1,2]
```
...
## 9. スティープネス
F=0の時の初期資源量から理論上求められる加入量R0に対する、0.2B0の時の加入量Rの比であるスティープネスhを計算します。


```{r,warning=FALSE}
year <- as.character(max(res_vpa$input$rec.year))
res_steepness <- calc_steepness(SR=resSR$input$SR,rec_pars=resSR$pars,M=res_vpa$input$dat$M[,year],waa=res_vpa$input$dat$waa[,year],maa=res_vpa$input$dat$maa[,year])
res_steepness$h
```
