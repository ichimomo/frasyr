...
- モデル診断は重要なので、ある程度網羅的に実施できるスクリプトや関数の使用例を配布します
- 基本的には「チューニングあり」VPAが対象ですが、「感度分析」と「レトロスペクティブ解析」はチューニングなしVPAでも実施できます
- ここで用いているデータ（vpadat_base0）は非常に単純なダミーデータです
...
- リッジVPA
  - リッジVPAで最終年最高齢のFのみ推定(`sel.update=TRUE`) : 選択率更新法の代わりにリッジVPAで全F推定法を行います
  - リッジVPAで最終年のFを全推定(`term.f="all"`) : 全F推定の代わりにリッジVPAで選択率更新法を行います
...
# チューニングなしVPAの場合
res_tmp <- vpa(vpadat_base0, tf.year=2015:2016, last.catch.zero = FALSE, 
               Pope = TRUE, p.init = 0.5, tune=FALSE)
tmp <-   do_sensitivity_vpa(what_replace = "tuning", value = list(2015:2016, 2014:2016, 2013:2016))
...
tmp <- do_sensitivity_vpa(res_tmp, what_replace = "tuning", value = list(2015:2016))
...
- 最終年のFを全推定(term.F="all")していれば、選択率更新法を感度分析で行います
- 最終年最高齢のFのみ推定を(sel.update=TRUE)していれば、全F推定を感度分析で行います


リッジVPAのペナルティ(lambda)について感度分析したい場合は、`index = "lambda"`としてください。（次のセクションを参照）
...
# b推定しているが、bを固定する場合
tmp <- do_sensitivity_vpa(res_vpa_example, what_replace = "b", value = list(c(1,1),c(1.2,1.2))) 
# bを固定しているが、bを推定する場合
...
res_tmp <- vpa(vpadat_base0, tf.year=2015:2016, last.catch.zero = FALSE, Pope = TRUE, 
...
以降、チューニングありのみの診断法となります。
以下の例では、資源評価最終年のFをすべて推定した場合も関数が正常に動くことを示すために、全F推定法（`term.F = "all"`）として、解析しました。

また、`est.method = "ml"`と最尤法での推定となっていますが、以下の関数は最小二乗法`est.method = "ls"`でも機能します。
...
初期値は対数を取った0.001から4の間を20刻みにした後、指数を取ってます。刻みの数は`n_ite`で指定できます。
...
res_vpa_jitter <- do_estcheck_vpa(res_vpa_example)
...
ここで赤い横線は与えた結果の推定値と尤度を表している。

結果の図を見ると、初期値に対して推定結果、尤度ともにほぼ変化せず（プロットが横一直線）、結果の推定値および尤度と重なっている（赤線上にプロットがある）ことから、推定パラメータは初期値に対して頑健であることが確認できました。
（**ここではダミーデータを与えているため、推定値が初期値に応じて多少ばらついているが、尤度はほぼ変化がないことが分かる**）
...
plot_example <- plot_residual_vpa(res_vpa_example, plot_year = 2010:2017)
...
また時間の都合上、生成するブートストラップ標本数は50個としていますが、95%信頼区間などを正確に計算したい場合は1000回（デフォルト）ほど必要です。
...
このようにSSB、加入量、バイオマスそれぞれについて95％信頼区間とともに資源動態の作図ができます。（**ダミーデータなので信頼区間が変な結果となっています**）
...
res_tmp <- numeric()
for(i in 1:50) res_tmp[i] <- as.numeric(res_vpa_bootstrap$res_boot[[i]]$Fc.at.age[4])
