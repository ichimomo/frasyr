...
Type objective_function<Type>::operator()() {
  ...
  for (int i = 0; i < nsim; i++) {  // input initial number
    for (int t = 0; t < future_initial_year + 1; t++) {
      // この時点で、future_initial_year前までにnaaがデータがあることを確認
      for (int iage = 0; iage < nage; iage++) {
        N_mat(iage, t, i) = naa_mat(iage, t, i);
        catch_mat(iage, t, i) = caa_mat(iage, t, i);
        spawner_mat(t, i) +=
            N_mat(iage, t, i) * waa_mat(iage, t, i) * maa_mat(iage, t, i);
      }
    }
  }
  ...
  for (int i = 0; i < nsim; i++) {
    for (int t = future_initial_year - 1; t < total_nyear; t++) {
      ...
      for (int a = 1; a < nage; a++) {
        spawner_mat(t, i) +=
            N_mat(a, t, i) * waa_mat(a, t, i) * maa_mat(a, t, i);
      }
      ...
      if (t > future_initial_year - 1) {
        ...
        if (SR_mat(t, i, 3) == 2) {  // Beverton-Holt
          N_mat(0, t, i) =
              SR_mat(t, i, 0) * spawner_mat(t - recruit_age, i) /
              (1 + SR_mat(t, i, 1) * spawner_mat(t - recruit_age, i));
        }
        if (SR_mat(t, i, 3) == 3) {  // Ricker
          N_mat(0, t, i) =
              SR_mat(t, i, 0) * spawner_mat(t - recruit_age, i) *
              exp(-SR_mat(t, i, 1) * spawner_mat(t - recruit_age, i));
        }
      ...
      if (t < (total_nyear - 1)) {
        for (int iage = 0; iage < (nage - 1); iage++) {
          N_mat(iage + 1, t + 1, i) =
              N_mat(iage, t, i) * exp(-M_mat(iage, t, i) - F_mat(iage, t, i));
        }
        N_mat(nage - 1, t + 1, i) +=
            N_mat(nage - 1, t, i) *
            exp(-M_mat(nage - 1, t, i) - F_mat(nage - 1, t, i));
      }
    }
  }
  ...
  for (int i = 0; i < nsim; i++) {
    for (int t = 0; t < total_nyear; t++) {
      for (int a = 0; a < nage; a++) {
        if (Pope) {
          catch_mat(a, t, i) = waa_catch_mat(a, t, i) * N_mat(a, t, i) *
                               exp(-Type(0.5) * M_mat(a, t, i)) *
                               (1 - exp(-F_mat(a, t, i)));
        } else {
          catch_mat(a, t, i) = waa_catch_mat(a, t, i) * N_mat(a, t, i) *
                               (1 - exp(-M_mat(a, t, i) - F_mat(a, t, i))) *
                               F_mat(a, t, i) /
                               (M_mat(a, t, i) + F_mat(a, t, i));
        }
      }
    }
  }
