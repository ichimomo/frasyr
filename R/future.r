    ...
    function(res0,
        ...
        if(!start.year%in%years){
            ...
            rec.tmp <- recfunc(thisyear.ssb[1,],res0,
                               rec.resample=rec.tmp$rec.resample,rec.seed=arglist$seed,
                               rec.arg=rec.arg)
               ...
               && is.null(faa.new)) # <- pre.catchで漁獲量をセットしていない
            {
                if(!isTRUE(use.MSE)){
                    ...
                    ABC.tmp <- get_ABC_inMSE(naa_all,waa_all,maa_all,faa_all,M[,(i-2):(i),],res0,
                                             ...
                                             N=MSE.options$N,
                                             recfunc=MSE.options$recfunc,rec.seed=arglist$seed,
...
get_ABC_inMSE <- function(naa_all,waa_all,maa_all,faa_all,M,res0,start_year,nyear,N,recfunc,rec.seed,rec.arg,Pope,HCR,
...
# Beverton-Holt
BH.recAR <- function(ssb,vpares,deterministic=FALSE,rec.resample=NULL,rec.seed=NULL,
...
RI.recAR <- function(ssb,vpares,deterministic=FALSE,rec.resample=NULL,rec.seed=NULL,
                   rec.arg=list(a=1000,b=1000,sd=0.1,bias.correction=TRUE)){
...
HS.recAR2 <- function(ssb,vpares,#deterministic=FALSE,
                     rec.resample=NULL,rec.seed=NULL,
...
# Beverton-Holt
BH.recAR2 <- function(ssb,vpares,deterministic=FALSE,rec.resample=NULL,rec.seed=NULL,
...
# Ricker
RI.recAR2 <- function(ssb,vpares,deterministic=FALSE,rec.resample=NULL,rec.seed=NULL,
                     rec.arg=list(a=1000,b=1000,sd=0.1,rho=0,c=1,bias.correction=TRUE)){
  a <- rec.arg$a
  b <- rec.arg$b
  c <- rec.arg$c
