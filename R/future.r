...
future_vpa_R <- function(naa_mat,
  ...
  for(t in future_initial_year:total_nyear){
    ...
    #OS 上行に変更
      ...
      if(!is.na(HCR_mat[t,,"TAC_reserve_amount"])){HCR_mat[t,,"expect_wcatch"] <- max(0, (as.numeric(HCR_realized[t,,"original_ABC_plus"]) - as.numeriv(HCR_mat[t,,"TAC_reserve_amount"])))} #OS修正 なおマイナスにならないようにする
      if(t<total_nyear){
