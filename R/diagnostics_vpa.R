...
do_sensitivity_vpa <- function(res, what_replace, value, what_plot = NULL, ncol = 5){
  ...
  if(what_replace == "M"){
    ...
    for(i in 1:length(value)){
      ...
      res_vpa.s[[i]] <- safe_call(vpa, input0, force=T)
    ...
    if (res$input$tune == FALSE){
      ...
      input0$sel.update <- FALSE
      input0$term.F <- "all"
      res_vpa.s[[1]] <- safe_call(vpa, input0, force=T)
      lab.tmp <- "ALL_F.est"
    ...
    if(res$input$b.est == TRUE){
      ...
      for(j in 1:length(value)){
        ...
        res_vpa.s[[j]] <- safe_call(vpa, input0, force=T)
...
do_estcheck_vpa <- function(res, n_ite = 20, sd_jitter = 1, what_plot = NULL, TMB = FALSE){
  ...
  for (i in 1:n_ite) {
    ...
    tmp <- try(safe_call(vpa, input0, force=T))
...
do_jackknife_vpa <- function(res, method = "index", what_plot = NULL, ncol = 5){
  ...
  if(method == "index"){
    ...
    for(i in 1:length(name_tmp)){
      ...
      res_tmp <- safe_call(vpa, input0, force=T)  # vpa関数の実行
...
do_caaboot_vpa <-  function(res, B_ite = 1000, B_cv = 0.2, ci_range = 0.95){
  ...
  for(i in 1:B_ite){
    ...
    res_tmp <- try(safe_call(vpa, input0, force=T))
